...
cmodel1 <- lm(committee ~ dollar_amount + current_party + state + largefirm + topfirm, data = data)
summary(cmodel1)
...
summary(pmodel)
#Model w/ party + state controls
pmodel1 <- lm(passed ~ dollar_amount + current_party + state + largefirm + prevpos + topfirm, data = data)
summary(pmodel1)
#Model w/ sponsor controls but neither state nor party
...
dataviz <- dataviz[, c("bill_name", "status", "current_party", "state", "sponsor", "numlobbygroups", "numfilings", "committee", "passed", "totalspent")]
...
summary(cm)
#Model w/ party + state controls
cm1 <- lm(committee ~ totalspent + current_party + state + numlobbygroups + numfilings, data = dataviz)
summary(cm1)
#Model w/ sponsor controls but neither state nor party
...
Creating unique dummies for each lobby group:
```{r}
...
```{r}
#check means of bills that passed for each status
dataviz %>% 
  group_by(status) %>% 
  summarize(meanstat = mean(totalspent))
```
