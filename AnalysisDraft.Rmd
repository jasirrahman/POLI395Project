...
data <- data%>% 
  mutate(committee = ifelse(status > 1, 1, 0))  #1 if out of committee 0 if not out
#2nd is for distinguishing between 1+2 and 4s
data <- data%>% 
  mutate(passed = ifelse(status > 2, 1, 0))  #1 if passed 0 if didn't pass
#Model
model1 <- lm(committee ~ dollar_amount + sponsor + current_party + state + registrant_name, data = data)
summary(model1)
model2 <- lm(passed ~ dollar_amount + sponsor + current_party + state + registrant_name, data = data)
summary(model2)
```
